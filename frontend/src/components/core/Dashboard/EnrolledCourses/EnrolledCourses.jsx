                ...
                {/* Cards shure hote h ab */}
                {
                  enrolledCourses.map((course, index) => (
                    <div key={index} onClick={() => {
                      ...
                      <div className='flex w-2/5 flex-col gap-2 py-3 mx-auto'>
                        {
                          progressData?.map((progress, index) => {
                            //show 0 progress if no progress data is available
                            console.log(progress.courseID, course._id);
                            if (progress.courseID === course._id) {
                              console.log(progress);
                              console.log(progress?.completedVideos?.length, totalNoOfLectures(course))
                              return (
                                <div key={index}>
